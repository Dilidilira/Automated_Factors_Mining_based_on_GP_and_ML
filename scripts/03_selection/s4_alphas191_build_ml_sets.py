...
if __name__ == "__main__":
    ...
    alphas191_index=pd.read_csv('trees/Non_redundant_factors.csv')
...
#    merged_df = merge_alpha_dataframes(alpha_dict)
    ...
    merged_df=pd.read_csv('factors/alpha191/alphas191_data.csv')
    # 设置训练集的时间范围
    train_time_ranges = [
    ...
    # 设置y_train, y_test
    returns=pd.read_csv(f'data_returns/{freq}Returns.csv')
    ...
    #保存训练集测试集
    X_train.to_csv('trees/X_train_selected.csv')
    X_test.to_csv('trees/X_test_selected.csv')
    Y_train.to_csv('trees/Y_train_selected.csv')
    Y_test.to_csv('trees/Y_test_selected.csv')
